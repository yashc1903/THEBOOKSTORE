...
function SellProduct() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:8080/product/sell-product', productData);
      if (data?.success) {
        ...
        <div className="flex flex-col justify-center items-center border w-full md:w-3/4">
          <div className="text-center w-full max-w-4xl">
            ...
            <div className="flex justify-center mt-4">
              {photo && <img src={URL.createObjectURL(photo)} alt="" className="mt-4" height={'100px'} />}
            </div>
            <div>
              <input
                type="text"
              ...
              <textarea
                ...
                className="text-xl w-full p-4 text-center mt-4 border-4 text-black"
              ...
              <button className="mt-2 border-2 rounded-lg bg-emerald-600 text-white p-4 text-2xl" onClick={handleCreate}>
                CREATE PRODUCT
              </button>
            </div>
          </div>
